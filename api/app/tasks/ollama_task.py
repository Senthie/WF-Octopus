...
from app.services import CeleryTaskRecordService
...
def ollama_generate(
    self, prompt: str, model: str = 'llama2', image_base64: str = '', task_record_id: str = ''
):
    """
    ...
    print(f'ollama_generate START {task_id}')
    ...
    async def _run():
        ...
        try:
            payload = {
                ...
                'prompt': prompt,
                'stream': False,
                'images': [image_base64] if len(image_base64) > 0 else [],  # 注意是列表
            }
            # 调用 Ollama
            resp = httpx.post(
                f'{settings.ollama_host}/api/generate',  # 使用默认端口 11434
                json=payload,
                timeout=15.0,
            )
